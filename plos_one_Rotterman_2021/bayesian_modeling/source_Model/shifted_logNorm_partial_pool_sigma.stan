...
parameters {
  ...
  real<lower=0> sigma_1;  // pooled standard deviation
...
generated quantities {
  ...
  real SOL = ndt_2 + exp(mu_2 + 0.5*sigma_1^2);

  for (i in 1:N1) {
    y1rep[i] = lognormal_rng(mu_1, sigma) + ndt_1;
  }
  for (i in 1:N2) {
    y2rep[i] = lognormal_rng(mu_2, sigma_1) + ndt_2;
  }
